...
router.get("/listar", (req, res) => {
    conexao.query(
      "SELECT * FROM dbadega.produto WHERE tipo='Cerveja'",
      (erro, resultado) => {
        if (erro)
  ...
  router.get("/buscar/tipo", (req, res) => {
    conexao.query(
      "SELECT * FROM produto WHERE tipo='Cerveja' ORDER BY tipo='Cerveja' DESC",
      [req.params.id],
      (erro, resultado) => {
        if (erro)
  ...
  router.post("/cadastrar", verificarToken, (req, res) => {
    conexao.query("INSERT INTO produto SET tipo='Cerveja'", [req.body], (erro, resultado) => {
      if (erro)
        return res.status(500).send({ retorno: `Erro interno -> ${erro}` });
  ...
  router.put("/atualizar/:tipo", verificarToken, (req, res) => {
    conexao.query(
      "UPDATE produto SET ? WHERE tipo='Cerveja'",
