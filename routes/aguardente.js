...
router.get("/listar", (req, res) => {
    conexao.query(
      "SELECT * FROM dbadega.produto WHERE tipo='Aguardente'",
      (erro, resultado) => {
        if (erro)
  ...
  router.get("/buscar/tipo", (req, res) => {
    conexao.query(
      "SELECT * FROM produto WHERE tipo='Aguardente' ORDER BY tipo='Aguardente' DESC",
      [req.params.id],
      (erro, resultado) => {
        if (erro)
  ...
  router.post("/cadastrar", verificarToken, (req, res) => {
    conexao.query("INSERT INTO produto SET tipo='Aguardente'", [req.body], (erro, resultado) => {
      if (erro)
        return res.status(500).send({ retorno: `Erro interno -> ${erro}` });
  ...
  router.put("/atualizar/:tipo", verificarToken, (req, res) => {
    conexao.query(
      "UPDATE produto SET ? WHERE tipo='Aguardente'",
